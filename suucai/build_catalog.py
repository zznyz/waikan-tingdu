...
POSTED = {
    ...
    "高考外刊|1","高考外刊|2","高考外刊|3","高考外刊|4","高考外刊|5","高考外刊|6","高考外刊|7","高考外刊|8","高考外刊|9","高考外刊|10","高考外刊|11",
    # 初中外刊 — No.09 韦布望远镜 07-23 Elaine 确认已发（No.10 二十四节气 视频已交付, 未发）
...
PRODUCTS = [
    ...
    #    文件本身是好的：12 个码逐个访问 12/12 通、12 个地址互不重复、每篇各扫各的（当天实测）。
    ("外刊", "高中外刊精读 · 下篇 No.19–30", "高中外刊精读-合订本下篇-19-30-v3.pdf", "134 页", "08-02 00:28",
     "old", "old", "🔴 <b>网盘里还没有这一本</b>（2026-08-02 她自查发现）⇒ 买过的人一直只拿到上篇。文件是现成好的，直接挂 <code>v3</code>，⛔ 别挂 v1/v2（v2 一个二维码都没有）。<br>挂之前自查：翻到 <b>第 3 页</b>（深蓝篇章封面）→ 下方有<b>白色扫码卡</b>＝v3；没有＝旧版。⛔ 别看第 4 页原文页，新版那页本来就没码"),
