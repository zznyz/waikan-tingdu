...
POSTED = {
    ...
    #        🆕 2026-09-03 Elaine 原话「**高考听力昨天发的已经发了**」
    #           ⇒ `gaokao-zibian01`（自编第01组前5题，09-02 发）标已发。
    #           ⚠️ **d3（08-22 那条）＝状态未知，⛔ 不是「未发」** —— 2026-09-03 问她，她原话「8.22...我真不记得了」。
    #              ⇒ 表里没有『未知』这个值，就先留在 POSTED 之外；**⛔ 别把它当成『确认未发』去推荐重发**，
    #                也别擅自标已发。她哪天顺手在小红书翻一眼再定。
    ...
    #              🔴 **2026-08-17 15:xx 视频号已补发，本期两个平台都齐了** —— 她原话「**视频号补棉花糖吧现在**」
    #                → 我把视频（97.0s / 5.5M，ffprobe 验过）+ 视频号版文案推给她 → 她回「**视频号已发**」。
    ...
    #   🔴 No.11 绿海龟 2026-08-04 12:3x Elaine 原话「绿毛龟视频外刊精读初中已发小红书，
    #     记录素材平台已发」⇒ 标已发。**12:5x 她补了一句「视频号已发」**
    #     ⇒ **No.11 是两个平台都发了的**（小红书 + 视频号），⛔ 别再当"视频号待补"。
    #     ⚠️ 这条也说明：先确认一个平台不等于另一个没发，**隔一会儿她可能补齐** ——
    #       所以注释里那句"某平台未确认"要跟着她的后续话更新，⛔ 别一直挂着。
    #   🔴 No.12 沙克尔顿（坚忍号/航海）2026-08-06 20:2x Elaine 原话
    #     「**外刊今天中考这篇航海的，小红书已发**」⇒ 标已发。
    #     🔴 **2026-08-08 17:5x 她主动确认：视频号这一期确实没发** —— 原话
    #       「**视频号的绿乌龟和丝绸之路中间那个的没发，小红书发了**」
    #       （"绿乌龟和丝绸之路中间那个" ＝ No.11 与 No.13 之间 ＝ **No.12 沙克尔顿**）。
    #       🔴 **2026-08-18 03:2x（北京）她再次确认仍没发**：原话「**沙克尔顿明天白天补，提醒我一声**」
    #         ⇒ 已挂提醒（scheduler `task-msxmb6gd-941pd0`，北京 08-18 11:30 ＝ UTC 03:30）。
    #         ⚠️ 同一轮她说过「视频号拖延了…初中到丝绸之路」——**⛔ 别据此认为 12 也发了**，
    #           她紧接着自己讲明 12 还要补。**这正是本节那条"绝不能从发到第几期推断中间期"的又一次实证。**
    #       ⇒ 从"未确认"升级为"确认未发"。素材当天已发她（跟读视频 61.64s + 视频号版文案），
    #       ⛔ 补发不算重复。⚠️ 这一期 hero 是 Frank Hurley 的 **Public domain**，
    #         **不需要署名行** —— 08-07 定的"12 期要署名"是给 CC 授权那批的，别混。
    #     📌 **她认期数靠内容不靠编号**（"绿乌龟""中间那个"）⇒ 回她时先把编号翻译成篇名再确认。
    #   🔴 No.13 丝绸之路 2026-08-08 17:4x Elaine 原话「**小红书和视频号贴图号已发**」⇒ 标已发。
    #     **这一期三个平台都发了**（小红书 + 视频号 + 贴图号），⛔ 不用再补任何平台 ——
...
PAN_UPDATED = {
    ...
    # 🆕 2026-09-04 00:32 BJT 她回「也挂网盘了」⇒ pan 从 unknown 翻成 ok。
    #    ⚠️ 记的是**她确认的时刻**，⛔ 不等于上传时刻（她没说几点传的，不许替她精确到分）。
    "高考精编听力": "09-04 00:32 确认（她原话「也挂网盘了」）",
}
...
PRODUCTS = [
    ...
    # 🆕 2026-09-03 高考精编听力上架（¥29.9）。她当天两次亲口确认「已经上架了」/「Anyway，听力已经上架了」
    #    ⇒ sale 填 ok 合规（本文件规矩：**只认她亲口说，⛔ 不许推断**）。
    #    ⚠️ pan 留 unknown —— 她**没说过**挂没挂网盘，⛔ 别替她填 ok（下篇那次假 ✅ 的教训就在上面）。
    ("听力", "高考精编听力", "高考精编听力.pdf", "62 页", "09-03 08:20",
     "ok", "ok", "翻到 <b>第 1 页</b>封面：写「<b>高考精编听力</b>」且「160 道往年真题 · 20 道模拟题 · 共 180 道」＝新版；写「高考听力 · 真题精练」或「6 套真题 120 道题」＝旧版。<br>⛔ 对外截图一律用 <code>高考精编听力-无码样张.pdf</code>，别从这份发货版截（第 3/6/9/12/15/18/21/24/27 页有活码）"),
]
